...
//button间隔
static const CGFloat ButtonMargin = 17.0f;
//button标题正常大小
static const CGFloat ButtonFontNormalSize = 14.0f;
//button标题选中大小
static const CGFloat ButtonFontSelectedSize = 17.0f;
//顶部ScrollView高度
static const CGFloat TopScrollViewHeight = 40.0f;

@interface GQSegmentButton ()<UIScrollViewDelegate>
{
    //界面滑动的ScrollView
...
@implementation GQSegmentButton
...
-(void)buildUI
{
    ...
    _lineView.frame = CGRectMake(0, _topScrollView.bounds.size.height, self.bounds.size.width, 0);
...
-(void)updateUIWithSelectedIndex:(NSInteger)index byButton:(BOOL)byButton
{
    ...
    [UIView animateWithDuration:0.35 animations:^{
        ...
        if ([_delegate respondsToSelector:@selector(segmentDidselectTab:)]) {
            [_delegate segmentDidselectTab:index];
        }
        if (byButton) {
            //更新主ScrollView范围
